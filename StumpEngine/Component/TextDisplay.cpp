...
TextDisplay::TextDisplay(GameObject& game_object, const std::string& font, std::string_view text, const float size) :
    Component(game_object), m_font(TTF_OpenFont(font.c_str(), size)), m_font_size(size), m_text(std::move(text)),
...
void TextDisplay::render(SDL_Renderer* renderer)
{
    ...
    const SDL_FRect dest_location{pos.x, pos.y, static_cast<float>(m_text_texture->w),
